...
void loop() {
  ...
  if (microscope) {
    ...
    unsigned long green_current_millis = millis();
    unsigned long red_current_millis = millis();
  
    // Start green blinker
    if (green_current_millis - green_previous_millis >= interval) {
      green_previous_millis = green_current_millis;
    ...
    if (red_current_millis - red_previous_millis >= interval) {
      red_previous_millis = red_current_millis;
...
void draw_text_static(const char *text) {
  ...
  // https://forum.arduino.cc/t/adafruit-oled-how-to-center-text/617181/4
  int16_t x1, y1;
  uint16_t w, h;
  int scale_factor = 1;
